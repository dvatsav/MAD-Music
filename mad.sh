...
play_video()
{
	...
	if [ $ans = 'i' ]
		...
		then
			echo
			echo "Enter the name of the playlist (without extension"
...
playlist_ops()
{
	...
	if [ $answer = 'c' ]
		then
			...
			ls Playlists
			echo
			echo "Enter the name of the playlist you wish to modify (without extension)"
			echo
			echo -n "-> "
			read play_mod
			clear
			...
			do 
				...
				if [ $ans_mod = 'd' ]
					then
						clear
						echo "Enter the name of the song you wish to delete: "
						echo
...
display_files()
{
	...
	echo
	ls | grep ".mkv\|.mp3\|.mp4\|.wav\|.webm\|.avi" 
#	ls | grep ".mp3" 
#	ls | grep ".mp4" 
#	ls | grep ".wav"
#	ls | grep ".webm" 
